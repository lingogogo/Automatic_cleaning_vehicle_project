...
void demo(char *outfile,char *cfgfile, char *weightfile, float thresh, float hier_thresh, int cam_index, const char *filename, char **names, int classes, int avgframes,
...
/*    if (outfile) {
    ...
    //}
     // char *tmp = "[\n";
      //fwrite(sizeof(char), strlen(tmp), json_file); 
    } */
    ...
    while(1){
	    ...
	    json_file = fopen(outfile, "wb");
            ...
            json_buf = detection_to_json(dets, nboxes, l.classes, names, json_image_id, "data");

            fwrite(json_buf, sizeof(char), strlen(json_buf), json_file);
	    char *end = "\n]";
	    fwrite(end,sizeof(char), strlen(end), json_file);
            free(json_buf);
	    fclose(json_file);
	    //rewind(json_file);
	    //ftruncate(json_file,50000);
    	    }
