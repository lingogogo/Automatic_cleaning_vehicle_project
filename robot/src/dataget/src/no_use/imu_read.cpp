/*
 HI219 factory default output protocol receiving:
 Output sum = 41
 0x5A+0xA5+LEN_LOW+LEN_HIGH+CRC_LOW+CRC_HIGH+ 0x90+ID (1 byte) + 0xA0+Acc (acceleration 6 bytes) + 0xB0+Gyo (angular velocity 6 bytes) + 0xC0+Mag (geomagnetic 6 bytes) + 0xD0 +AtdE (Euler angle 6 bytes) + 0xF0+Pressure (pressure 4 bytes)
*/
#include "ros/ros.h"
...
int main(int argc, char **argv)
{
    ...
    ros::NodeHandle n;
         //Publish topic, message format uses sensor_msg::Imu standard format (topic name, queue length)
    ...
    while (ros::ok())
    {
        ...
        if(buf_tmp[0] == 0x5A )
        {
            ...
            /* 
                         Receive according to the factory default output protocol:
                         0x5A+0xA5+LEN_LOW+LEN_HIGH+CRC_LOW+CRC_HIGH+ 0x90+ID (1 byte) + 0xA0+Acc (acceleration 6 bytes) + 0xB0+Gyo (angular velocity 6 bytes) + 0xC0+Mag (geomagnetic 6 bytes) + 0xD0 +AtdE (Euler angle 6 bytes) + 0xF0+Pressure (pressure 4 bytes)
            */
            int i=0;
